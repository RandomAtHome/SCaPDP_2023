    ...
    for compile_prefix in compile_prefixes:
        ...
        for compiler_optimizer in compiler_optimizers:
            ...
            proc = subprocess.Popen(
                "{0} -{1} {2}.c -o {2}_{1}".format(compile_prefix, compiler_optimizer, prog_name), 
                # base args
                shell=True,
                executable="/bin/bash", 
            ...
            print("RUN COMMAND: {0} -{1} {2}.c -o {2}_{1} --- avgt: {3}".format(compile_prefix, compiler_optimizer, prog_name, full_time / n_tr))
